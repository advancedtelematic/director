...
trait NamespacedRequests extends DirectorSpec with DefaultPatience with RouteResourceSpec {
  ...
  def findUpdateMetadata(device: DeviceId, target: TargetFilename)(implicit ns: NamespaceTag): Json =
    Get(apiUri(s"device/${device.show}/targets/${target.value}/metadata")).namespaced ~> routes ~> check {
      status shouldBe StatusCodes.OK
      responseAs[Json]
    }
}
