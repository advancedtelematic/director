...
import com.advancedtelematic.director.db.{UpdateTargetRepositorySupport, DeviceRepositorySupport, Errors => DBErrors,
...
class Roles(rolesGeneration: RolesGeneration)
           (implicit db: Database, ec: ExecutionContext) extends UpdateTargetRepositorySupport
  ...
  private def nextVersionToFetch(ns: Namespace, device: DeviceId, currentVersion: Int): Future[Int] = {
    ...
    updateTargetRepository.updateExists(ns, device, timestampVersion).flatMap {
