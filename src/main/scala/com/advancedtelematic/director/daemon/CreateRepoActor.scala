...
class CreateRepoWorker(directorRepo: DirectorRepo, defaultKeyType: KeyType)(implicit ec: ExecutionContext) {
  private lazy val _log = LoggerFactory.getLogger(this.getClass)

  def action(uc: UserCreated): Future[Done] = {
    val namespace = Namespace(uc.id)

    directorRepo.findOrCreate(namespace, defaultKeyType).map(_ => Done).recover {
